...
type TGetCurrentPlaylist = (accessToken: string) => Promise<Playlist>;

export const getCurrentPlaylist: TGetCurrentPlaylist = async (accessToken) => {
  const requestOptions: AxiosRequestConfig<any> = {
    headers: buildHeaders(accessToken),
  };

  const endPoint: string = `${config.SPOTIFY_BASE_URL}/me/playlists`;
  const response: AxiosResponse = await axios.post(endPoint, requestOptions);
